...
- 一个用于调试和排障的 Web 控制台
- 一个用于 workspace 选择、session 对话、流式输出和本地运行时调试的终端 TUI
...
| 层级 | 名称 | 定位 |
| --- | --- | --- |
| `OAS` | Open Agent Spec | 实际用户层，特指用户导入的 tools、skills、model entries、`AGENTS.md`、`MEMORY.md` 以及其他 workspace 级增量配置。 |
| `OAR` | Open Agent Runtime | 面向开发者的运行时包层，由平台 / runtime 开发者构建和发布可复用的 runtime bundle，用来初始化 workspace。 |
| `OAH` | Open Agent Harness | 企业/平台部署形态，面向 Compose 或 Kubernetes、PostgreSQL、Redis、对象存储、controller、sandbox fleet 和多 worker。 |
| `OAP` | Open Agent Harness Personal | 个人部署形态，面向本地 daemon、SQLite、本地磁盘、embedded worker 和单用户工作流。 |

Web 调试端、CLI、TUI 和后续 Electron 桌面端都连接同一套 OAH-compatible API；它们既可以连接企业服务，也可以连接本地个人守护进程。
...
它也明确了作者边界：开发者发布 OAR runtimes，实际用户把 OAS specs 带入自己的 workspace。
客户端应先读取 server profile，例如 `GET /api/v1/system/profile`，再决定启用 OAH-only 或 OAP-only 行为。
...
- 面向终端的 debug TUI，可用于 workspace/session 检视和流式对话调试
...
如果你更常在终端里工作，OAH 也提供了基于 Ink 的调试 TUI：

<p align="center">
  <img src="assets/tui-screenshot.png" width="820" alt="OAH Debug TUI 截图" />
</p>

TUI 使用的仍然是同一套 API 与 SSE 接口。它适合在 repo 或 shell 内直接选择 workspace、创建或恢复 session、查看流式输出，并在不打开浏览器的情况下做本地运行时调试。
